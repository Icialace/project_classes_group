"""
Hao Zhang Zheng, Job Landicho, Barnabas Orosz. Dec. 11, 2022.
The program takes numerical inputs to navigate through a menu.
Through each of the submenu, there are a variety of options.
These options include viewing data, finding specific data based on an input,
and some of the submenus allow editing of data. The outputs of these selections will
display the requested details through a table. Inputs that are written to a file
are formatted according to the format of the file. Outputs taken from the file are read
and then formatted to be displayed via a table.
This inputs and outputs are dependant on the data files that are provided.
"""
from tabulate import tabulate

# Doctor Class and Functions
class Doctor:
  #formatDrInfo formats a new input to prepare to write to a file.
  def formatDrInfo(new_doctor):
    new_doctor_info = '_'.join(new_doctor)
    new_doctor_info = "\n" + new_doctor_info
    return new_doctor_info
  
  #enterDrInfo allows a user to input data for a new doctor.
  def enterDrInfo():
    new_doctor = []
  ...
  #readDoctorsFile opens the doctors.txt file and assigns it to a variable
  def readDoctorsFile():
    with open("/content/doctors.txt", mode = "r") as doctor_open:
      doctor_raw_data = doctor_open.readlines()
    return doctor_raw_data

  #searchDoctorByID allows a user to input an ID to give the info required.
  ...
  #searchDoctorByName allows a user to input a name to give the info required.
  ...
  #displayDoctorInfo displays formatted information of a single doctor in the form of a table
  ...
  #editDoctorInfo allows a user to edit information of a doctor based on their ID
  def editDoctorInfo():
    edit_doctor = []
  ...
  #displayDoctorsList displays the formatted doctor data in the form of a table.
  ...
  #writeListOfDoctorsToFile writes a list of doctors to a file from another file
  ...
  #addDrToFile appends information of a single doctor to a file
